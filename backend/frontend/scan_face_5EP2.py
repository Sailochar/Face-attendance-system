...
def current_session(classroom, timetable):
    ...
    for session_name, times in timetable.get(classroom, {}).items():
...
def log_to_excel(classroom, name, role, session, date_str, times):
    ...
    filename = f"{classroom}_{month}_{year}.xlsx"
    ...
    if os.path.exists(filepath):
        ...
        ws.title = month
        ws.append(
            ["Role", "Name", "Roll No", "Class"] +
            [f"Day {d} In" for d in range(1, 32)] +
            [f"Day {d} Out" for d in range(1, 32)]
        )

    ws = wb[month] if month in wb.sheetnames else wb.create_sheet(month)
    ...
    ws.cell(row=found_row, column=4 + day).value = times["in"]  # In time
    ws.cell(row=found_row, column=4 + 31 + day).value = times["out"]  # Out time
...
def mark_attendance(classroom, name, role, timetable):
    ...
    if role == "faculty":
        if name not in session_records[key]["faculty"]:
            ...
            session_records[key]["faculty"][name] = {
                "in": datetime.now().strftime("%H:%M"),
                "out": ""  # Out time empty on arrival
            }
            status = "Present"
        else:
            # Subsequent time - update out time only
    ...
    # Student logic
        ...
        if session_records[key]["faculty_ended"]:
            return session, "Fake Attendance"  # faculty ended session, no more attendance
        if name not in session_records[key]["students"]:
            session_records[key]["students"][name] = {
                "in": datetime.now().strftime("%H:%M"),
                "out": ""  # Out time empty first
            }
            status = "Present"
        else:
...
def draw_panel(frame, classroom, session, status, name):
    panel_height = 60
    cv2.rectangle(frame, (0, frame.shape[0] - panel_height), (frame.shape[1], frame.shape[0]), (30,30,30), -1)
        ...
        if session and encodings:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if True in matches:
                    ...
                    color = (0,255,0) if "Present" in status else (0,255,255) if "Half" in status else (0,0,255)
